...
# start.date<-min(data$"Camera Trap"_start_date)
# end.date<-max(data$"Camera Trap"_end_date)
...
f.calendar.yr <- function(dataset,yr_toplot) { ##########start ploting function
  ...
  start.date<-min(as.Date(dataset$"Camera_Trap_Start_Date"))
  end.date<-max(as.Date(dataset$"Camera_Trap_End_Date"))
  dates<-seq.Date(start.date, end.date ,by = "years")
  yr<-as.numeric(substr(dates, 1, 4))
  no.yr<-(max(yr)-min(yr))+1
  
  years<- as.numeric(substr(as.Date(dataset$"Photo_Date"),1,4))
  months<- as.numeric(substr(as.Date(dataset$"Photo_Date"),6,7))
  days<- as.numeric(substr(as.Date(dataset$"Photo_Date"),9,10))
  ...
  cameras<-as.character(unique(x=data2$"Camera_Trap_Name"))
  camvec<-unique(x=data2$"Camera_Trap_Name")
  ...
  list.cams.ini.fin<-list() #inicializa la lista que guarda inicio fin
  for (z in 1:ncam){
    
    allyrs_cam<-subset(data2,Camera_Trap_Name==c(cameras[z])) # camara 1 todos los anios
    # secfotocam<-as.data.frame(allyrs_cam$photo_date2) # secuencia de fechas de camara1
    perdaycam<-as.data.frame(table(allyrs_cam$Photo_Date, dnn="Photo_Date")) #numero de fotos por dia cam1
    perdaycam$Photo_Type<-"Photo"
    
    #add fecha inicio de cameras [1]
    cam01.start<-as.data.frame(unique(subsub<-subset(data2,Camera_Trap_Name ==cameras[z],select=Date.Start))) 
    cam01.start$Freq<-1
    cam01.start$Photo_Type<-"Start"
    colnames(cam01.start)<-c("Date.End","Freq","Photo_Type")
    cam01.end<-as.data.frame(unique(subsub<-subset(data2,Camera_Trap_Name ==cameras[z],select=Date.End)))
    cam01.end$Freq<-1
    cam01.end$Photo_Type<-"End"
    ...
    colnames(mat.start.end.cam01)<-c("Photo_Date","Freq","Photo_Type")
    
    mat.cam01<-rbind(perdaycam,mat.start.end.cam01) #paste camara 
    mat.cam01$"Camera_Trap_Name"<-cameras[z]
  ...
  # the year is simple
  data4$year<-as.numeric(as.POSIXlt(as.Date(data4$Photo_Date))$year+1900)
  
  # the month too 
  data4$month<-as.numeric(as.POSIXlt(as.Date(data4$Photo_Date))$mon+1)
  ...
  data4$weekday <-  as.POSIXlt(as.Date(data4$Photo_Date))$wday
  ...
  data4$yearmonth<-as.yearmon(as.Date(data4$Photo_Date))
  data4$yearmonthf<-factor(data4$yearmonth)
  # then find the "week of year" for each day
  data4$week <- week(as.Date(data4$Photo_Date)) ###########
  # and now for each monthblock we normalize the week to start at 1 
  data4<-ddply(data4,.(yearmonthf),transform,monthweek=1+week-min(week))
  # add dia del anio
  data4$dayofmonth<-days(data4$Photo_Date)
  # add unos
  data4$one<-1
  ### fechas de lubridate
  data4$week2 = as.POSIXlt(as.Date(data4$Photo_Date))$yday %/% 7 + 1
  data4$wday2 = as.POSIXlt(as.Date(data4$Photo_Date))$wday
  data4$dayofyr <- yday(as.Date(data4$Photo_Date))
  ...
  cameras<-unique(x=yr09$"Camera_Trap_Name")
  ...
  yr09_array1<-yr09 #subset(yr09,"Camera_Trap_Name" %in% c(camvec[1:camera.max.per.yr])) #Subset de varias camaras en 1 yr
  ...
  momintext<- str_sub(as.character(month(ymd(080101) + months(momin), label = TRUE, abbr = TRUE)), 1, -3)
  nextmonth1<-str_sub(as.character(month(ymd(080101) + months(momin+2), label = TRUE, abbr = TRUE)), 1, -3)
  nextmonth2<-str_sub(as.character(month(ymd(080101) + months(momin+3), label = TRUE, abbr = TRUE)), 1, -3)
...
#   )
  ...
  if (modif==3) {rect_left<-c(monthstart1,monthstart2)
                 ...
                 cal1<- ggplot() +  
                   geom_rect(data=rectangles, aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax), 
                             fill="gray80", alpha=0.5) + 
                   annotate("text", x = monthstart1+15, y = 1, label = labelmonth1, colour = "gray75",size = 4) +
                   annotate("text", x = monthstart1+75, y = 1, label = labelmonth2, colour = "gray75",size = 4) #+
                 # annotate("text", x = monthstart1+138, y = 1, label = labelmonth3, colour = "gray75",size = 5) 
                  }
  ...
  cal2<-cal1 + geom_tile(data=yr09_array1, aes(dayofyr,one, fill = Photo_Type)) +   
    scale_fill_manual(guide = guide_legend(title = "Record Type"),values=c("red", "steelblue", "green")) 
  
  cal3<- cal2 + facet_grid(Camera_Trap_Name ~.) +
  ...
  cal5<- cal4 + theme(strip.text.y = element_text(colour = "black", angle = 0, size = 7,
                                                  hjust = 0.5, vjust = 0.5)) +
