...
f.matrix.creator2<-function(data,year){
  ...
  cams<-unique(data$Camera_Trap_Name)
  cams<-sort(cams)
  rows<-length(cams)
  species<-unique(data$binomial)
  # start and end dates of sampling periods
  # data<-data[data$Sampling.Period==year,]
  min<-min(data$Camera_Trap_Start_Date, na.rm = TRUE)
  max<-max(data$Camera_Trap_End_Date, na.rm = TRUE)
  ...
  start.dates<-tapply(as.character(data$Camera_Trap_Start_Date),data$Camera_Trap_Name,unique)
  nms<-names(start.dates)
  # start.dates<-ymd(start.dates)
  names(start.dates)<-nms
  end.dates<-tapply(as.character(data$Camera_Trap_End_Date),data$Camera_Trap_Name,unique)
  ...
  for(i in 1:length(species)){
    ...
    cameras<-data$Camera_Trap_Name[indx]
...
f.matrix.creator3 <- function(data,year){
  ...
  for(i in 1:length(species)){
    ...
    #fill in the matrix
    for(j in 1:length(dates.cameras[,1])){
      col<-which(date.header==date(dates.cameras[j,1]))
      row<-which(cams==dates.cameras[j,2])
      mat[row,col]<-1
    }
    mat.nas<-is.na(mat)
